...
template<typename T, template<class...> typename A, template<class...> typename S, template<class...> typename M>
...
template<typename T, template<class...> typename A, template<class...> typename S, template<class...> typename M>
inline bool dep_sort<T, A, S, M>::add_dependency(const T& node, const T& dependency) {
  if (dependency == node) {
    return false;
  }
...
inline bool dep_sort<T, A, S, M>::add_dependencies(const T& node, const C<T>& dependencies) {
...
template<typename T, template<class...> typename A, template<class...> typename S, template<class...> typename M>
inline typename dep_sort<T, A, S, M>::result_type dep_sort<T, A, S, M>::sort() const {
...
template<typename T, template<class...> typename A, template<class...> typename S, template<class...> typename M>
inline bool dep_sort<T, A, S, M>::has_dependency(const value_type& node, const value_type& dependency) const {
  if (has_node(node)) {
...
inline void dep_sort<T, A, S, M>::sort(result_type& result) {
  ...
  for (const auto& [node, relations] : map_) {
